...
def buildExonTable(db, chr, transcript_id2strand, five_utr_dict,
                   three_utr_dict, start_codon_dict, stop_codon_dict, cds_dict,
    ...
    component_list = [
        five_utr_dict, three_utr_dict, start_codon_dict, stop_codon_dict,
        cds_dict, exon_dict
    ]
    ...
    for txt_id in transcript_id2strand:
        ...
        if txt_id in exon_dict:
            ...
            print("Transcript does not have a CDS or exon feature: %s" %
                  txt_id)
            sys.exit(1)
        ...
        for coord in exon_coords:
            if coord[start_idx] == 0:
                ...
                sys.exit(1)
            exon_tuple = getExonTuple(db, txt_id, gene_name, chr,
                                      coord[start_idx], coord[stop_idx],
                                      strand, classification, db_name)
            exon_table_rows.append(exon_tuple)

    # Now insert all into the table
    db.insertListIntoTable(
        "exon",
...
def buildGeneTable(db, db_name, chr):
    ...
    for row in cg_nums:
        ...
        start_records = db.getDBRecords_Dict(start_select_state,
                                             db_name,
                                             check_same_thread=False)
        ...
        end_records = db.getDBRecords_Dict(end_select_state,
                                           db_name,
                                           check_same_thread=False)
...
def extractInsertSeqs(tbl_name, db_obj, genome_file_name, db_name):
    ...
    for tempSeqObj in SeqIO.parse(seq_file, "fasta"):
        ...
        select_statement = """SELECT * FROM %s
                              WHERE chr=\'%s\'""" % (tbl_name, chr_name)

        records = db_obj.getDBRecords_Dict(select_statement,
                                           db_name,
                                           check_same_thread=False)
        ...
        for row in records:
            ...
            if tbl_name == "exon":
                ...
                update_statement = """UPDATE intron
                                      SET watson_strand_sequence=\'%s\'
                                      WHERE intron_id=%d""" % (
                    seq, int(row["intron_id"]))

            db_obj.updateTable(tbl_name,
                               update_statement,
                               db_name,
                               check_same_thread=False)
    ...
    for i in range(len(dict_list)):
        if txt_id in dict_list[i]:
            for gff_obj in dict_list[i][txt_id]:
                blocks.append((int(gff_obj.start), int(gff_obj.end)))
...
def inferIntrons(db, db_name, chr):
    ...
    txt_records = db.getDBRecords_Dict(txt_select,
                                       db_name,
                                       check_same_thread=False)
    ...
    for row in txt_records:
        ...
        exon_records = db.getDBRecords_Dict(exon_select,
                                            db_name,
                                            check_same_thread=False)
...
def getExonTuple(db, txt_id, gene_name, chr, start, end, strand,
                 classification, db_name):
    ...
    for txt_id in table_dict:
        for gff_obj in table_dict[txt_id]:
            ...
            select_statement = """SELECT exon_id FROM exon
                                  ...
                                  start<=%d
                                  AND 
                                  end>=%d""" % (this_txt_id, int(
                gff_obj.start), int(gff_obj.end))
            record = db.getDBRow_Dict(select_statement,
                                      db_name,
                                      check_same_thread=False)
            if record is None:
            ...
            if rowExists(db, db_name, row_exist_statement):
                print("This exon_id has multiple associated {}: {}".format(table, exon_id), end=' ')
                print(" in %s" % db_name)
                continue

            cols = "exon_id, start, end"
            vals = "%d,%d,%d" % (exon_id, int(gff_obj.start), int(gff_obj.end))
            db.insertIntoTable(table,
                               cols,
                               vals,
                               db_name,
                               check_same_thread=False)
...
def mergeConnectBlocks(blocks):
    ...
    exonic_locations = list(exonic_dict.keys())
...
def rowExists(db, db_name, select_statement):
    ...
    row = db.getDBRow_Dict(select_statement, db_name, check_same_thread=False)
