...
def main():
    ...
    for event in table2:
        ...
        if subset_file:
            ...
            table2_events.add((lineList[1], lineList[3], lineList[4],
                               lineList[5], lineList[6]))
    ...
    for as_type in as_type2chr2strand2redundantGroup2event:
        for chr in as_type2chr2strand2redundantGroup2event[as_type]:
            for strand in as_type2chr2strand2redundantGroup2event[as_type][
                    chr]:
                for rGroup in as_type2chr2strand2redundantGroup2event[as_type][
                        chr][strand]:
                    which_tables, pvals = getTableNums(
                        as_type2chr2strand2redundantGroup2event[as_type][chr]
                        [strand][rGroup], compare_pval)
                    if which_tables == [1, 2]:
                        this_event = as_type2chr2strand2redundantGroup2event[
                            as_type][chr][strand][rGroup].pop()
                        ...
                        if compare_pval:
                            out_1_and_2.write("\t%.3f\t%.3f" %
                                              (pvals[0], pvals[1]))
                        out_1_and_2.write("\n")
                    elif which_tables == [1]:
                        for this_event in as_type2chr2strand2redundantGroup2event[
                                as_type][chr][strand][rGroup]:
                            this_event_list = this_event.split("\t")
    ...
    if subset_file:
        ...
        for line in table1:
            ...
            lineList = line.split("\t")
            if (lineList[1], lineList[3], lineList[4], lineList[5],
...
# FUNCTIONS #
#############
def buildDictionary(as_type2chr2strand2redundantGroup2event, event,
                    which_table):
...
def get_rGroup_as_event(event):
    ...
    if as_type == "mutually_exclusive":
        ...
        group_start, group_end = findLargestRegion(",".join(
