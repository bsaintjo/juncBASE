...
#           raw_right_input_file_name = "%s_right_intron_counts.txt" % prefix
    ...
    if html_out_dir:
        exec("import rpy2.robjects.lib.ggplot2 as ggplot2", globals())
...
#                   continue
        ...
        if k_means:
            (sample_set1, sample_set2) = find_clusters(event, all_psis,
                                                       idx2sample)
        (batch2setLabels, batch2len) = buildBatchDict(sampleList, samp2batch,
        ...
        if which_test == "wilcox.test":
            event_type2PSI_vals_4_set[event_type].append(
                (robjects.r['median'](robjects.FloatVector(set1_psis))[0],
                 robjects.r['median'](robjects.FloatVector(set2_psis))[0]))
        else:
            event_type2PSI_vals_4_set[event_type].append(
                (robjects.r['mean'](robjects.FloatVector(set1_psis))[0],
        ...
        try:
            if permutation:
                ...
                null_dist = get_null_dist(line_list[samp_start_idx:],
                                          total_counts, all_psis, which_test,
                                          batch2setLabels, batch2len,
                                          sum(map(ord, event)),
                                          samp_set_thresh1, samp_set_thresh2)

                this_stat = robjects.r[which_test](
                    robjects.FloatVector(set1_psis),
                ...
                raw_pval = robjects.r[which_test](
                    robjects.FloatVector(set1_psis),
        ...
        for event in left_events2counts:
            ...
            for j in range(total_samples):
                ...
                if simple_IR:
                    ...
                    total_excl = int(
                        round((left_col_excl + right_col_excl) / 2.0))
...
#                   continue
                ...
                if simple_IR:
                    (total_psi, total_sum_ct) = getPSI_sample_sum(
                        "%d;%d" % (total_excl, total_incl), sum_thresh)
            ...
            if k_means:
                (sample_set1,
                 sample_set2) = find_clusters(event, all_psis, idx2sample)
            (batch2setLabels,
             batch2len) = buildBatchDict(sampleList, samp2batch, sample_set1,
                                         sample_set2)
            ...
            try:
                if permutation:
                    if simple_IR:
                        ...
                        this_stat = robjects.r[which_test](
                            robjects.FloatVector(set1_total_psis),
                        ...
                        this_stat = robjects.r[which_test](
                            robjects.FloatVector(set1_psis_left),
            ...
            if simple_IR:
                ...
                if robjects.r["is.nan"](left_pval)[0] or robjects.r["is.nan"](
                        right_pval)[0]:
                    continue
                else:
    ...
    for event_type in event_type2pvals:
        if as_only:
            ...
            event_type2adjusted_pvals[event_type] = robjects.r['p.adjust'](
                robjects.FloatVector(event_type2pvals[event_type]), method)
    ...
    if as_only:
        ...
        if which_test == "wilcox.test":
            ...
            all_psi_output.write(
                "\tset1_mean\tset2_mean\tdelta_val\traw_pval\tcorrected_pval\n"
            )
...
def find_clusters(event, all_psis, idx2sample):
    ...
    clusters = list(robjects.r["kmeans"](robjects.FloatVector(non_NA_all_psis),
                                         2)[0])
...
def get_null_dist(excl_incl_counts, total_counts, all_psis, which_test,
                  batch2setLabels, batch2len, random_seed, samp_set_thresh1,
                  samp_set_thresh2):
    """ 
    ...
    for i in range(0, NUM_ITERATIONS, NUM_IN_BATCH):
        ...
        for excl_incl_count in excl_incl_counts:
            iter_ctr += 1
            excl, incl = list(map(int, excl_incl_count.split(";")))
            ...
            idx2incl_iter.append(
                list(
                    vgam.rbetabinom_ab(NUM_IN_BATCH, tot_count, incl + 1,
                                       excl + 1)))
        ...
        for k in range(NUM_IN_BATCH):
            ...
            while ctr < MAX_ITERATION:
                ...
                for batch in batch2setLabels:
                    ...
                    random.seed(random_seed + sum(map(ord, batch)) + iter_ctr +
                                k + ctr)
...
#                       sys.exit(1)
            ...
            stats.append(robjects.r[which_test](
                robjects.FloatVector(this_set1_psis),
