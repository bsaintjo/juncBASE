...
def createTmpJcnBed(input_dir, intron_junction2strand, by_chr):
    ...
    if by_chr:
        for this_chr in intron_junction2strand:
            chr2jcn_bed[
                this_chr] = "%stmp_%s_preProcess_getASEventReadCounts_step2.bed" % (
                    input_dir, this_chr)
    else:
        chr2jcn_bed[
            "all"] = input_dir + "tmp_preProcess_getASEventReadCounts_step2.bed"
    ...
    for this_chr in chr2jcn_bed:
        ...
        for jcn_str in this_intron_junction2strand:
            (chr, chrStart, chrEnd, blockLens,
             secondBlockStart) = parse_jcn_str(jcn_str)

            bed_line = "%s\t%d\t%d\t%s\t%d\t%s\t%d\t%d\t0,0,0\t2\t%d,%d\t0,%d\n" % (
                chr, chrStart, chrEnd, jcn_str, PSEUDO_COUNT,
                this_intron_junction2strand[jcn_str], chrStart, chrEnd,
                blockLens, blockLens, secondBlockStart)
        ...
        if by_chr:
            ...
            for jcn_str in jcn_str2strand:
                if jcn_str in intron_junction2strand:
                    if jcn_str2strand[jcn_str] != intron_junction2strand[
                            jcn_str]:
...
def getJunctionStrs(bed_file):
    ...
    for line in bed_file:
        ...
        blocks = list(map(int, line_list[10].split(",")))
        block_starts = list(map(int, line_list[11].split(",")))
