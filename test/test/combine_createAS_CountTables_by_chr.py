...
def main():
    ...
    lenNorm_output_file = open(
        prefix + "_AS_exclusion_inclusion_counts_lenNorm.txt", "w")
    lenNorm_left_intron_file = open(prefix + "_left_intron_counts_lenNorm.txt",
                                    "w")
    lenNorm_right_intron_file = open(
        prefix + "_right_intron_counts_lenNorm.txt", "w")
    ...
    header = grabHeader(
        root_dir +
        "/tmp_createAS_CountTables_%s_AS_exclusion_inclusion_counts.txt" %
        chr_list[0])
    ...
    for this_chr in chr_list:
        ...
        if (len(chr_out.readlines()) == 1) or (
                len(chr_out_lenNorm_lines) == 1):
        ...
        if (len(chr_left_intron_lines) == 1) or (
                len(chr_left_intron_lenNorm_lines) == 1):
        ...
        if (len(chr_right_intron_lines) == 1) or (
                len(chr_right_intron_lenNorm_lines) == 1):
