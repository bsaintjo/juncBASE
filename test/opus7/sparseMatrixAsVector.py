...
class SparseMatrixAsVector(SparseMatrix):
    ...
    def __setitem__(self, indices, value):
        ...
        if position >= 0:
            ...
            if len(self._array) == self._numberOfElements:
                ...
                for p in range(len(self._array)):
                    newArray[p] = self._array[p]
                for p in range(len(self._array), len(newArray)):
    ...
    def putZero(self, i, j):
        ...
        if position >= 0:
            self._numberOfElements -= 1
            for k in range(position, self._numberOfElements):
                self._array[k] = self._array[k + 1]
    ...
    def getTranspose(self):
        ...
        for i in range(self._numberOfColumns):
            offset[i] = 0
        for i in range(self._numberOfElements):
    ...
    def __mul__(self, mat):
        ...
        matT = mat.transpose
        result = SparseMatrixAsVector(self._numberOfRows, matT.numberOfRows,
