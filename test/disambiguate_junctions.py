...
def main():
    ...
    if by_chr:
        ...
        for this_chr in chr_list:
            ...
            for this_samp in samples:
                bed_file_name = "%s/%s/%s_%s/%s_%s_junctions.bed" % (
                    input_dir, this_samp, this_samp, this_chr, this_samp,
                    this_chr)

                # Copy old bed file to keep raw information.
                original_bed_name = "%s/%s/%s_%s/%s_%s_junctions_original.bed" % (
                    input_dir, this_samp, this_samp, this_chr, this_samp,
                    this_chr)
                ...
                for line in original_bed:
                    ...
                    if majority_rules:
                        updateJcn2Strands(jcn2strands, line_list[3],
                                          line_list[5])
            ...
            if majority_rules:
                ...
                if fixed_jcn2strand != {}:
                    for this_samp in samples:
                        bed_file_name = "%s/%s/%s_%s/%s_%s_junctions.bed" % (
                            input_dir, this_samp, this_samp, this_chr,
                            this_samp, this_chr)
    ...
    # Files are just organized by sample
        ...
        jcn2strands = {}
        for this_samp in samples:
            bed_file_name = "%s/%s/%s_junctions.bed" % (input_dir, this_samp,
                                                        this_samp)

            original_bed_name = "%s/%s/%s_junctions_original.bed" % (
                input_dir, this_samp, this_samp)
            ...
            for line in original_bed:
                ...
                if majority_rules:
                    updateJcn2Strands(jcn2strands, line_list[3], line_list[5])
        ...
        if majority_rules:
            ...
            if fixed_jcn2strand != {}:
                for this_samp in samples:
                    bed_file_name = "%s/%s/%s_junctions.bed" % (
                        input_dir, this_samp, this_samp)
                    bed_file = open(bed_file_name)
                    bedlines = bed_file.readlines()
                    bed_file.close()
