...
def main():
    ...
    while changeOccurred:
        changeOccurred = mergeRedundantEvents(
            as_type2chr2strand2redundantGroup2event)

    # Now remove redundant events from as_type2event2pval dictionary
    removeRedundantEvents(as_type2chr2strand2redundantGroup2event,
                          as_type2event2pval)
...
# FUNCTIONS #
#############
def buildDictionaries(event, as_type2event2pval,
                      as_type2chr2strand2redundantGroup2event, use_mad):
...
def removeRedundantEvents(as_type2chr2strand2redundantGroup2event,
                          as_type2event2pval):
    """
    ...
    for as_type in as_type2chr2strand2redundantGroup2event:
        for chr in as_type2chr2strand2redundantGroup2event[as_type]:
            for strand in as_type2chr2strand2redundantGroup2event[as_type][
                    chr]:
                for redundant_group in as_type2chr2strand2redundantGroup2event[
                        as_type][chr][strand]:
                    if len(as_type2chr2strand2redundantGroup2event[as_type]
                           [chr][strand][redundant_group]) > 1:
                        most_sign_event = findMostSignEvent(
                            as_type2event2pval[as_type],
                            as_type2chr2strand2redundantGroup2event[as_type]
                            [chr][strand][redundant_group])

                        for event in as_type2chr2strand2redundantGroup2event[
                                as_type][chr][strand][redundant_group]:
