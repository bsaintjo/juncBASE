...
if not os.path.exists(IE_SCRIPT):
    print(
        "ERROR: intron_exon_jctn_counts.py needs to be in the same directory.")
    sys.exit(1)
SHELL = "/bin/tcsh"
...
def main():
    ...
    if paired_reads:
        if os.path.exists(paired_genome_file_name):
            paired_read_set = update_len2readFile(len2readFile,
                                                  readFile2handle, input_dir,
                                                  samp,
                                                  paired_genome_file_name,
                                                  True)
        else:
            print(
                ("Cannot find paired end file: %s" % paired_genome_file_name))
            sys.exit(1)

    if single_reads:
        update_len2readFile(len2readFile, readFile2handle, input_dir, samp,
                            genome_file_name)
    ...
    if forced_read_len:
        if len(len2readFile) > 1:
            ...
            opt_parser.print_help()
            sys.exit(1)
        if forced_read_len != list(len2readFile.keys())[0]:
            print((
                "Read length in file (%d) does not match specified read length (%d)."
                % (list(len2readFile.keys())[0], forced_read_len)))
            opt_parser.print_help()
            sys.exit(1)
    ...
    all_introns = combineCounts(input_dir, list(len2readFile.keys()), samp)

    # Find paired reads and create file associating paired reads.
    if paired_reads:
        make_paired_end_ie_junctions2qname(all_introns, input_dir,
                                           list(len2readFile.keys()),
...
def combineCounts(input_dir, read_lengths, samp):
    ...
    for read_len in read_lengths:
        len_count_file_name = input_dir + samp + "/tmp_" + samp + "_" + repr(
            read_len) + "_intron_exon_junction_counts.txt"
        ...
        rm_cmd = "rm " + input_dir + samp + "/tmp_" + samp + "_" + repr(
            read_len) + "_intron_exon_junction_counts.txt"
        os.system(rm_cmd)

        rm_cmd = "rm " + input_dir + samp + "/tmp_" + repr(
            read_len) + "_reads.txt.gz"
        ...
        rm_cmd = "rm " + input_dir + samp + "/tmp_" + samp + "_" + repr(
            read_len) + "_intron_exon_junction_coords.out"
        #        runCmd(rm_cmd, SHELL, True)
        os.system(rm_cmd)
    ...
    return list(intron2counts.keys())
...
def getTheseRegionCoords(all_introns, read_length, overhang):
    ...
    for intron in all_introns:
        ...
        left_coord = "%s_%d_%d" % (chr, intron_start - flanking_dist,
                                   intron_start + flanking_dist - 1)
        right_coord = "%s_%d_%d" % (chr, intron_end - flanking_dist + 1,
                                    intron_end + flanking_dist)

        region_coord2ie[left_coord] = "%s_%d_%d" % (chr, intron_start - 1,
                                                    intron_start)
        region_coord2ie[right_coord] = "%s_%d_%d" % (chr, intron_end,
                                                     intron_end + 1)
...
def update_len2readFile(len2readFile,
                        readFile2handle,
                        input_dir,
                        samp,
                        genome_file_name,
