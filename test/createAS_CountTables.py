...
def main():
    ...
    for line in first_file:
        ...
        event_key, raw_count_str, lenNorm_count_str, genes, strand, const_region = getKeyandCount(
            line)

        event2sample2raw_counts[event_key] = {first_sample: raw_count_str}
        event2sample2lenNorm_counts[event_key] = {
            first_sample: lenNorm_count_str
        }
        ...
        if "intron_retention" in line:
            left_raw_count_str, right_raw_count_str = getIRKeyandCounts(
                line, jcn_seq_len, False)
            left_lenNorm_count_str, right_lenNorm_count_str = getIRKeyandCounts(
                line, jcn_seq_len, True)
            left_intron2sample2raw_counts[event_key] = {
                first_sample: left_raw_count_str
            }
            right_intron2sample2raw_counts[event_key] = {
                first_sample: right_raw_count_str
            }
            left_intron2sample2lenNorm_counts[event_key] = {
                first_sample: left_lenNorm_count_str
            }
            right_intron2sample2lenNorm_counts[event_key] = {
                first_sample: right_lenNorm_count_str
            }
        ...
        for line in samp_file:
            ...
            if "intron_retention" in line:
                ...
                left_lenNorm_count_str, right_lenNorm_count_str = getIRKeyandCounts(
                    line, jcn_seq_len, True)
                left_intron2sample2raw_counts[event_key][
                    samp] = left_raw_count_str
                right_intron2sample2raw_counts[event_key][
                    samp] = right_raw_count_str
                left_intron2sample2lenNorm_counts[event_key][
                    samp] = left_lenNorm_count_str
                right_intron2sample2lenNorm_counts[event_key][
                    samp] = right_lenNorm_count_str
    ...
    for event_key in event2sample2raw_counts:
        ...
        lenNorm_counts_list = [
            event2sample2lenNorm_counts[event_key][first_sample]
        ]
        ...
        for samp in samples:
            if samp in event2sample2raw_counts[event_key]:
                raw_counts_list.append(
                    event2sample2raw_counts[event_key][samp])
                lenNorm_counts_list.append(
                    event2sample2lenNorm_counts[event_key][samp])
...
#                psi_list.append(NA)
        ...
        if "intron_retention" in event_key:
            if not hasInclusionCounts(event_key, left_intron2sample2raw_counts,
                                      right_intron2sample2raw_counts):
...
#           psi_output_file.write(outline)
    ...
    for event_key in left_intron2sample2raw_counts:
        ...
        raw_counts_list = [
            left_intron2sample2raw_counts[event_key][first_sample]
        ]
        lenNorm_counts_list = [
            left_intron2sample2lenNorm_counts[event_key][first_sample]
        ]
        #        psi_list = [getPSI(left_intron2sample2counts[event_key][first_sample])]
        for samp in samples:
            if samp in left_intron2sample2raw_counts[event_key]:
                raw_counts_list.append(
                    left_intron2sample2raw_counts[event_key][samp])
                lenNorm_counts_list.append(
                    left_intron2sample2lenNorm_counts[event_key][samp])
...
#           psi_left_file.write(outline)
    ...
    for event_key in right_intron2sample2raw_counts:
        if event_key in skipped_ir_events:
            continue
        raw_counts_list = [
            right_intron2sample2raw_counts[event_key][first_sample]
        ]
        lenNorm_counts_list = [
            right_intron2sample2lenNorm_counts[event_key][first_sample]
        ]
        #        psi_list = [getPSI(right_intron2sample2counts[event_key][first_sample])]
        for samp in samples:
            if samp in right_intron2sample2raw_counts[event_key]:
                raw_counts_list.append(
                    right_intron2sample2raw_counts[event_key][samp])
                lenNorm_counts_list.append(
                    right_intron2sample2lenNorm_counts[event_key][samp])
...
def updateGeneStrand(event2genesStrand, event_key, genes, strand,
    ...
    try:
        ...
        # Key does not exist
        event2genesStrand[event_key] = (set([]), strand, const_region)
