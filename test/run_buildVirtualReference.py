...
if not os.path.exists(SCRIPT):
    print(
        "ERROR: buildVirtualReference_median.py needs to be in the same directory."
    )
    sys.exit(1)
...
def main():
    ...
    opt_parser.add_option("--total_thresh",
                          dest="total_thresh",
                          type="int",
                          help="Threshold for total number of counts. DEF=%d" %
                          DEF_THRESH,
                          default=DEF_THRESH)
    ...
    file_suffix_list = [
        "_AS_exclusion_inclusion_counts.txt",
    ...
    print(("Output files were created with the following prefix: %s" %
           out_prefix))
