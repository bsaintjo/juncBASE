...
#    annotated_introns = getAnnotatedIntronCoords(db, as_db, this_chr)
    ...
    for intron in left_intron2line:
        if intron not in right_intron2line:
            ...
            combined_line = combineLines(
                left_line_list[left_intron2line[intron]],
                right_line_list[right_intron2line[intron]])

            diff_direction_out.write(combined_line + "\t" + intron + "\n")

            # Print all events to allEvent file
            out_str = getAllEventInfoLine(combined_line, intron,
                                          annotated_exons_by_strand,
                                          annotated_introns, lengthNorm)
...
def getAllEventInfoLine(combined_line, intron, annotated_exons,
                        annotated_introns, lengthNorm):
    (e_or_i, gene_name, chr, strand, left_intron_start, right_intron_end,
     excl_cts_raw, excl_cts_lenNorm, ie_cts_raw,
     ie_cts_lenNorm) = getEventInfo(combined_line)
    ...
    ie_ct_list_raw = list(map(int, ie_cts_raw.split(";")))

    ie_ct_list_lenNorm = list(map(int, ie_cts_lenNorm.split(";")))
...
def getEventInfo(combined_line):
    ...
    ie_cts_raw = "%d;%d" % (ie_left_raw, ie_right_raw)
    ie_cts_lenNorm = "%d;%d" % (ie_left_lenNorm, ie_right_lenNorm)

    return (e_or_i, gene_name, chr, strand, left_intron_start,
            right_intron_end, excl_cts_raw, excl_cts_lenNorm, ie_cts_raw,
            ie_cts_lenNorm)
