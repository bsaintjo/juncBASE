...
# FUNCTIONS #
#############
def adjustRefCounts(event_str, jcn_seq_len, lengthNorm_total, ref_psi, excl,
                    incl):
    raw_total = excl + incl
    ...
    adj_incl = int(
        round((float(inclusion_isoform_len) / DEF_EXON_LEN_NORM) *
...
def buildDicts(lenNormalized_counts_file):
    ...
    for line in lenNormalized_counts_file:
        ...
        for counts in counts_list:
            ...
            [excl, incl] = list(map(int, counts.split(";")))
            total = excl + incl
            if total == 0:
                lenNormalized_counts_event2psis[event].append(NA)
            else:
...
def dPSI(psi_vals):
    ...
    if type(psi_vals) == type({}):
        this_psi_vals = list(psi_vals.values())
    else:
...
def recalculateRefPSI(col2psi_str, col2total_count, col2weights):
    ...
    if col2weights:
        ...
        median_total = int(
            round(robustats.weighted_median(total_vals, weights)))
    else:
