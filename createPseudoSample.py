...
def main():
    ...
    if by_chr:
        ...
        if chrs == []:
            print((
                "Missing bed files from preProcess_getASEventReadCounts_by_chr_step2.py. They should be in: %s"
                % input_dir))
            opt_parser.print_help()
            sys.exit(1)
    ...
    if "all" in chr2pseudo_dir:
        ...
        if not os.path.exists(bed_file):
            print((
                "Missing %s. Please run preProcess_getASEventReadCounts_step2.py"
                % bed_file))
            sys.exit(1)
    ...
    if "all" in chr2pseudo_dir:
        i_e_file = input_dir + "/%s/%s_intron_exon_junction_counts.txt" % (
            samp, samp)
        if not os.path.exists(i_e_file):
            print(("Missing file %s." % i_e_file))
            print((
                "Please run preProcess_getASEventReadCounts_step3.py on sample %s."
                % samp))
            sys.exit(1)
        ...
        for chr in chr2pseudo_dir:
            i_e_file = input_dir + "/%s/%s_%s/%s_%s_intron_exon_junction_counts.txt" % (
                samp, samp, chr, samp, chr)
            if not os.path.exists(i_e_file):
                print(("Missing file %s." % i_e_file))
                print((
                    "Please run preProcess_getASEventReadCounts_step3.py on sample %s and chr %s."
                    % (samp, chr)))
                sys.exit(1)
    ...
    if "all" in chr2pseudo_dir:
        ...
        for chr in chr2pseudo_dir:
            genome_file = input_dir + "/%s/%s_%s/%s_%s_genome_reads.txt.gz" % (
                samp, samp, chr, samp, chr)
            pseudo_file = chr2pseudo_dir[
                chr] + "/pseudo_%s_genome_reads.txt.gz" % chr
